...
const Relations = () => {
    ...
    const handleAddRelations = (e) => {
        ...
        location.state.membersList.slice(1).map((m, index) => {
            m.relationBetween = relations[index];
            m.isHead = false;
        });

        // console.log(location.state.membersList);
        axios.post('http://localhost:8083/members', location.state.membersList, {headers:{"Content-Type" : "application/json"}})
            .then(response => response.data)
            .then((response) => {
                history.push('/success', {familyid: response});
            });
    }
